...
\restrict bv0mD8P1k2jIMgSmmwCDEzE0Yphe9rOfVp620hp6uSWHKkuDsfulbNPcU91NiwT
...
CREATE TABLE public.bookings (
    ...
    CONSTRAINT bookings_status_check CHECK (((status)::text = ANY (ARRAY[('PENDING'::character varying)::text, ('CONFIRMED'::character varying)::text, ('CANCELLED'::character varying)::text, ('EXPIRED'::character varying)::text, ('ATTENDED'::character varying)::text])))
);
...
CREATE TABLE public.concerts (
    ...
    CONSTRAINT concerts_event_category_check CHECK (((event_category)::text = ANY (ARRAY[('CONCERT_LIVE_MUSIC'::character varying)::text, ('COMEDY_SHOW'::character varying)::text, ('STANDUP_SPECIAL'::character varying)::text, ('THEATRE_DRAMA'::character varying)::text, ('FOOD_DRINK_FESTIVAL'::character varying)::text, ('ART_EXHIBITION'::character varying)::text, ('SPORTS_EVENT'::character varying)::text, ('WORKSHOP_MASTERCLASS'::character varying)::text, ('FILM_SCREENING'::character varying)::text, ('CULTURAL_FESTIVAL'::character varying)::text]))),
    CONSTRAINT concerts_status_check CHECK (((status)::text = ANY (ARRAY[('DRAFT'::character varying)::text, ('PUBLISHED'::character varying)::text, ('SOLD_OUT'::character varying)::text, ('CANCELLED'::character varying)::text, ('POSTPONED'::character varying)::text, ('COMPLETED'::character varying)::text])))
);
...
CREATE TABLE public.group_bookings (
    ...
    CONSTRAINT group_bookings_status_check CHECK (((status)::text = ANY (ARRAY[('ACTIVE'::character varying)::text, ('COMPLETED'::character varying)::text, ('EXPIRED'::character varying)::text, ('CANCELLED'::character varying)::text])))
);
...
CREATE TABLE public.payments (
    ...
    updated_at timestamp(6) without time zone,
    booking_id uuid NOT NULL,
    CONSTRAINT payments_status_check CHECK (((status)::text = ANY (ARRAY[('INITIATED'::character varying)::text, ('SUCCESS'::character varying)::text, ('FAILED'::character varying)::text, ('PENDING'::character varying)::text, ('REFUNDED'::character varying)::text])))
);
...
CREATE TABLE public.seat_inventory (
    ...
    tier_id uuid NOT NULL,
    CONSTRAINT seat_invetory_status_check CHECK (((status)::text = ANY (ARRAY[('AVAILABLE'::character varying)::text, ('LOCKED'::character varying)::text, ('BOOKED'::character varying)::text])))
);
...
CREATE TABLE public.ticket_tiers (
    ...
    CONSTRAINT ticket_tiers_tier_status_check CHECK (((tier_status)::text = ANY (ARRAY[('UPCOMING'::character varying)::text, ('ACTIVE'::character varying)::text, ('SOLD_OUT'::character varying)::text, ('PAUSED'::character varying)::text, ('EXPIRED'::character varying)::text])))
);
...
CREATE TABLE public.venue_section (
    ...
    y_position real,
    venue_id uuid NOT NULL,
    CONSTRAINT venue_section_section_type_check CHECK (((section_type)::text = ANY (ARRAY[('GA'::character varying)::text, ('ASSIGNED'::character varying)::text])))
);
...
CREATE TABLE public.venues (
    ...
    CONSTRAINT venues_venue_type_check CHECK (((venue_type)::text = ANY (ARRAY[('ESTABLISHED'::character varying)::text, ('TEMPORARY'::character varying)::text])))
);
...
CREATE TABLE public.waitlist (
    ...
    tier_id uuid NOT NULL,
    user_id uuid NOT NULL,
    CONSTRAINT waitlist_status_check CHECK (((status)::text = ANY (ARRAY[('WAITING'::character varying)::text, ('NOTIFIED'::character varying)::text, ('CONVERTED'::character varying)::text, ('EXPIRED'::character varying)::text, ('CANCELLED'::character varying)::text])))
);
...
\unrestrict bv0mD8P1k2jIMgSmmwCDEzE0Yphe9rOfVp620hp6uSWHKkuDsfulbNPcU91NiwT
